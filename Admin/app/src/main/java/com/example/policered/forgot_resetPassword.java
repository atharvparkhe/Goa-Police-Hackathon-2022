...
public class forgot_resetPassword extends AppCompatActivity {
        ...
        btn_sendOtp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(ip_otp_email.getText().toString().matches("")){
                    Toast.makeText(forgot_resetPassword.this,"Insufficient data",Toast.LENGTH_SHORT).show();
                }
////                else{
////                    postDataUsingVolley();
////                }
...
//                    postData.put("password", "qbwknnhi");
                ...
                RequestQueue requestQueue = Volley.newRequestQueue(forgot_resetPassword.this);
                JsonObjectRequest jsonAbjectRequest = new JsonObjectRequest(Request.Method.POST,
                        "http://192.168.0.134:3456/api/police-head-reset/", postData, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {

                        try {
                            Toast.makeText(forgot_resetPassword.this,response.getString("message"),Toast.LENGTH_SHORT).show();
                            otp_Sent = true;
//                            if(response.)
        ...
        btn_verify.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(ip_otp.getText().toString().matches("")){
                    Toast.makeText(forgot_resetPassword.this,"Enter the otp",Toast.LENGTH_SHORT).show();
                    return;
                }
                if(!ip_new_password.getText().toString().matches(ip_verify_password.getText().toString())){
                    Toast.makeText(forgot_resetPassword.this,"Password not same",Toast.LENGTH_SHORT).show();
                    return;
                }
////                else{
////                    postDataUsingVolley();
...
//                            editor.apply();
                            ...
                            Intent intent = new Intent(forgot_resetPassword.this, MainActivity.class);
                            startActivity(intent);
//                            progressDialog.dismiss();
//                            finish();
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Log.d("VolleyDebug","Error retrieving data");
                        Toast.makeText(forgot_resetPassword.this,error.getMessage(),Toast.LENGTH_LONG).show();
                        if(error.networkResponse.statusCode == 400){
                            try {
                                String responseBody = new String(error.networkResponse.data, "utf-8");
                                JSONObject data = new JSONObject(responseBody);
